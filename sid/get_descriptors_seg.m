function [invardesc grd weights] = get_descriptors_seg(inim,settings,fc,X,Y,ev)
% [invardesc grd weights dist polar] = get_descriptors_seg(inim,settings,fc,X,Y,ev)
%
...
% Inputs
% inim          : input image
% settings      : SID settings plus the lambda parameter (see demo.m file)
...
%
% Outputs
% invardesc     : invariant descriptor
% grd           : grid used for descriptor construction
% weights       : segmentation masks
%
% Copyright (C) 2012 Iasonas Kokkinos, Eduard Trulls
% iasonas.kokkinos@ecp.fr, etrulls@iri.upc.edu
%
...
% hard-coded settings (to disable experimental/deprecated modes)
settings.masks_type = 1; % exponential
settings.use_l1_distance = 0; % always L2
settings.distance_type = 1;
...
assert(size(ev,1)==size(inim,1) && size(ev,2)==size(inim,2));
n_ev = size(ev,3);
...
%% smooth eigenvector maps
[dummy dummy dummy dummy sgs sc_sig] = get_grid(settings);
clear dummy;
csigma      = sgs*sc_sig;
cind        = 1:length(sgs);
cn          = length(cind);

% use padding to eliminate artifacts around the image border
padding = 200;
ev_s = zeros(size(ev,1),size(ev,2),size(ev,3),cn,'single');
for r=1:cn
	cu  = max(csigma(r),.5);
	%clear dx; dx  = single((cu))*single(anigauss(double(ev(:,:,1)),cu,cu, -90 , 0, 1));
	
	fsz = filter_size(cu);
	hf = gaussian_1d(cu, 0, fsz);
	im_padding2 = zeros(size(ev,1)+2*padding,size(ev,2)+2*padding);
	im_padding2(padding+1:padding+size(ev,1),padding+1:padding+size(ev,2)) = 1;
	im_padding2 = conv2(im_padding2,hf,'same');
	im_padding2 = conv2(im_padding2,hf','same');
	im_padding2 = im_padding2(padding+1:padding+size(ev,1),padding+1:padding+size(ev,2));
	for v=1:size(ev,3)
		im_padding1 = ev(:,:,v);
		im_padding1 = conv2(im_padding1,hf,'same');
		im_padding1 = conv2(im_padding1,hf','same');
		ev_s(:,:,v,r) = im_padding1 ./ im_padding2;
	end
end

%% compute descriptors
[desc dist weights] = mex_compute_all_descriptors_seg(dzy.H, dzy.params, dzy.ogrid(:,:,1), dzy.ostable, single(0),single(X'),single(Y'),single(ev_s));
if nargout<3, clear weights; end;
if nargout<4, clear dist; end;
desc = desc';
...
% this takes a lot of memory so don't do it if you can
if nargout>=3
	% reshape weights
	weights = reshape(weights,[nr,ns,size(weights,2)]);
end

invardesc               = get_desc(polar,settings.cmp,settings.invar);

%% dense descriptors: reshape to be in original image size
if ~isempty(fc)
    szd             = size(invardesc);
    szp             = size(polar);
    invardesc       = reshape(invardesc,[szd(1:end-1),sz(1),sz(2)]);
...
%rps  = zeros(size(feats_match),'single');
if (donorm==1)
    for sc = 1:ns,
        fsc    = feats_match(:,:,sc,:);
...
elseif (donorm==2)
	...
	% try to make this less resource-heavy
...
%%
% there's only mex code for SI/LI=1 for the seg-aware descriptors, so do not touch this
SI = 1;
LI = 1;
NT = 1;
...
dzy.params(11) = 0;
dzy.params(12) = settings.masks_type;
dzy.params(13) = settings.use_l1_distance;
dzy.params(14) = settings.distance_type;
dzy.params(15) = settings.lambda;
...
dzy.params(1)       = dzy.params(1) - 1*dzy.params(8);
